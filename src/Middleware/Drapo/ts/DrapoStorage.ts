class DrapoStorage {
    ...
    public async Retrieve(dataKey: string, sector: string, context: DrapoContext, dataKeyParts: string[] = null): Promise<DrapoStorageItem> {
    ...
    private IsDataKeyElement(dataKey: string, renderContext: DrapoRenderContext): boolean {
        ...
        const isDataKeyElement: boolean = this.Application.Searcher.HasDataKeyElement(dataKey);
    ...
    private async RetrieveDataItemInternal(dataKey: string, sector: string, canLoadDelay: boolean = false, dataDelayFields: string[] = null): Promise<DrapoStorageItem> {
        ...
        if (itemSystem !== null)
            return (itemSystem);
        const el: HTMLElement = this.Application.Searcher.FindDataKey(dataKey, sector);
        if (el == null) {
    ...
    private async ResolveValueMustaches(dataKey: string, sector: string, value: string): Promise<string> {
        if (value == null)
            return (null);
        return (await this.ResolveDataUrlMustaches(dataKey, sector, value, null, null));
    }

    private async ResolveValueMustachesAsNumber(dataKey: string, sector: string, value: string): Promise<number> {
        if (value == null)
            return (null);
        const valueResolved: string = await this.ResolveDataUrlMustaches(dataKey, sector, value, null, null);
        const valueAsNumber: number = this.Application.Parser.ParseNumber(valueResolved, null);
        ...
        for (let i: number = 0; i < dataItem.Channels.length; i++) {
            ...
            for (let j: number = 0; j < this._cacheItems.length; j++) {
                ...
                if (dataItem.Data === dataItemCurrent.Data)
                    continue;
                await this.Application.Storage.UpdateData(dataItemCurrent.DataKey, dataItemCurrent.Sector, dataItem.Data, true);
            }
        }
        return (true);
    }
    ...
    public GetCachedDataItemByDatePolling(): DrapoStorageItem {
    ...
    public ExistCachedDataItem(item: DrapoStorageItem): boolean {
        for (let i: number = this._cacheItems.length - 1; i >= 0; i--) {
            const storageItem: DrapoStorageItem = this._cacheItems[i];
            if (storageItem === item)
                return (true);
    ...
    public async ExecuteCachedDataItemPolling(item: DrapoStorageItem): Promise<void>{
        if (!item.IsTypeValue)
            return;
        await this.Application.FunctionHandler.ResolveFunctionWithoutContext(item.Sector, item.Element, (item.Data as any) as string);
    }
